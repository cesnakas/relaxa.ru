...
function barCodeGen( $productID ) {
  ...
  $kod .= (10 - ($sum % 10)) % 10;
   ...
   header("Content-Disposition: attachment; filename=relaxaPrice.xml");
...
while( $iBlock = $iBlocks->Fetch() ) {
   ...
   while($arSection = $rsSection->Fetch()) {
            ...
            $arFilter = array('IBLOCK_ID' => $iBlock['ID'], 'ACTIVE' => 'Y', 'IBLOCK_SECTION_ID' => $arSection["ID"], '!PROPERTY_DO_NOT_SHOW' => 'Y', '!CATALOG_QUANTITY' => '0');
            $elements = CIBlockElement::GetList(
                array(), $arFilter, false, false, array(
                    'ID',
                    'NAME',
   ...
   while ($element = $elements->GetNext()) {
	...
	if ($element['ID']!=$stop) {
      ...
      if ($element['PROPERTY_DOP_DISCOUNT_VALUE']) {
         $offers .= '<price>'.round($price['DISCOUNT_PRICE'] * 0.9432893).'</price>'.$NL;
      }
      else {
	  ...
	  if( $element['PROPERTY_MANUFACTURE_VALUE'] ) $offers .= '<country_of_origin>'.$element['PROPERTY_MANUFACTURE_VALUE'].'</country_of_origin>'.$NL;
