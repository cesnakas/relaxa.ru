...
<?if (!empty($arResult["ITEMS"])):?>
	...
	<div class="tab item">
							...
							<div class="item product" data-price-code="<?=implode("||", $arParams["PRICE_CODE"])?>">
								...
								<div class="tabloid">
									<?if(!empty($arElement["PROPERTIES"]["OFFERS"]["VALUE"])):?>
										<div class="markerContainer">
											<?foreach ($arElement["PROPERTIES"]["OFFERS"]["VALUE"] as $ifv => $marker):?>
											 	<div class="marker marker-<?=str_replace('#', '', $arElement["PROPERTIES"]["OFFERS"]["VALUE_XML_ID"][$ifv])?>"><?=$marker?></div>
											<?endforeach;?>
										</div>
									<?endif;?>
									...
									<?if(!empty($arElement["MIN_PRICE"])):?>
										...
										<span class="kredit_in_carousel">От <?=round($arElement["MIN_PRICE"]["DISCOUNT_VALUE"]/12)?>/мес.</span>
									...
									<?endif;?>
										...
										<div class="buy_in_carousel"><a href="#" class="addCart" data-id="<?=$arElement["ID"]?>">В корзину</a></div>
		...
		<script>
			/**/$("#topProduct-4").dwCarousel({
				leftButton: ".topBtnLeft-4",
				rightButton: ".topBtnRight-4",
				countElement: 5,
				resizeElement: true,
				resizeAutoParams: {
					1920: 4,
				...
				<a href="#" class="topBtnLeft-4"><</a>
				<a href="#" class="topBtnRight-4">></a>
	</div>
<?endif;?>