...
$resSection = CIBlockSection::GetList(array(), array('IBLOCK_ID' =>$iblockID, 'ID' => $sectionID), false, array("ID", "UF_ACTIONS"));
$sectionUF = $resSection->GetNext();
$actionElementId = $sectionUF["UF_ACTIONS"];

if ($actionElementId) {
    $actionRes = CIBlockElement::GetByID($actionElementId);
    $action = $actionRes->Fetch();
    $arResult['ACTION'] = array("PREVIEW" => $action["PREVIEW_TEXT"], "DETAIL" => $action["DETAIL_TEXT"]);
}

if (is_object($arResult["NAV_RESULT"])) {
    $arResult["NAVPAGE_COUNT"] = (int)$arResult["NAV_RESULT"]->NavPageCount;
} else {
    $arResult["NAVPAGE_COUNT"] = 1;
}
