...
<? if (!empty($arResult["ITEMS"])): ?>
    ...
    if ($arResult['ACTION']) {
        ...
        if ($APPLICATION->GetCurDir() != '/massazhnoe-oborudovanie/') { ?>
            <div style="width: 100%; background-color:white; text-align: center; position: relative; top: -24px; margin-bottom: -24px; font-size: 125%;">
                <?= $arResult['ACTION']['PREVIEW']; ?><span class='actionHead'
                                                            style="margin-left: 10px; cursor: pointer;"><img
                            src="/img/svg/arrow.svg" style="width: 14px;"></span>
            </div>
            <?
        } ?>
            ...
            <? foreach ($arResult["ITEMS"] as $index => $arElement): ?>
                ...
                <div class="item product sku" id="<?= $this->GetEditAreaId($arElement["ID"]); ?>"
                    ...
                    <div class="tabloid">
                        ...
                        <? if ($count > 1): ?>
                            <a href="<?= $arElement["DETAIL_PAGE_URL"] ?>#reviews" class="link">Читать отзывы
                                (<?= $count ?>)</a>
                        <? endif; ?>
                        ...
                        <? if ($url): ?>
                            <div class="optional">
                                <div class="row">
                                    <a href="<?= $url; ?>" class="addCart"><img
                                                src="<?= SITE_TEMPLATE_PATH ?>/images/incart.png" alt=""
                                                class="icon"><?= GetMessage("ADDCART_LABEL") ?></a>
                                    <a href="<?= $url; ?>" class="fastBack label"><img
                        ...
                        <? else: ?>
                            <div class="optional">
                                <div class="row">
                                    <a href="#"
                                       class="fastBack label<? if (empty($arElement["MIN_PRICE"]) || $arElement["CAN_BUY"] === "N" || $arElement["CAN_BUY"] === false): ?> disabled<? endif; ?>"
                                       data-id="<?= $arElement["ID"] ?>"><img
                                                src="<?= SITE_TEMPLATE_PATH ?>/images/fastBack.png" alt=""
                                                class="icon"><?= GetMessage("FASTBACK_LABEL") ?></a>
                                    <a href="#" class="addCompare label" data-id="<?= $arElement["ID"] ?>"><img
