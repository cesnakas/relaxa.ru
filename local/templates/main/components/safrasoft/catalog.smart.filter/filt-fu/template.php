...
<div id="smartFilter">
	...
	<form name="<?=$arResult["FILTER_NAME"]."_form"?>" action="<?=$arResult["FORM_ACTION"]?>" method="GET" id="smartFilterForm">
        ...
        if(!empty($arResult['NEW_ITEMS'])) {
            $resItems = $arResult['NEW_ITEMS'];
        } else {
            $resItems = $arResult['ITEMS'];
        }
		/*
		foreach($resItems as $key => $arItem)//prices
		{ 
				...
				<div class="paramsBox">
					...
					<div class="params">
						<div class="rangeSlider" id="sl_<?=$arItem["ID"]?>">
							<input name="<?=$arItem["VALUES"]["MIN"]["CONTROL_NAME"]?>" type="text" value="<?=empty($arItem["VALUES"]["MIN"]["HTML_VALUE"]) ? round($arItem["VALUES"]["MIN"]["VALUE"]) : round($arItem["VALUES"]["MIN"]["HTML_VALUE"])?>" id="<?=$arItem["VALUES"]["MIN"]["CONTROL_ID"]?>" onchange="smartFilter.keyup(this)" data-val="<?=round($arItem["VALUES"]["MIN"]["VALUE"])?>">
							<input name="<?=$arItem["VALUES"]["MAX"]["CONTROL_NAME"]?>" type="text" value="<?=empty($arItem["VALUES"]["MAX"]["HTML_VALUE"]) ? round($arItem["VALUES"]["MAX"]["VALUE"]) : round($arItem["VALUES"]["MAX"]["HTML_VALUE"])?>" id="<?=$arItem["VALUES"]["MAX"]["CONTROL_ID"]?>" onchange="smartFilter.keyup(this)" data-val="<?=round($arItem["VALUES"]["MAX"]["VALUE"])?>">
							<div class="slider">
								<div class="handler">
									<span class="min"><?=empty($arItem["VALUES"]["MIN"]["HTML_VALUE"]) ? round($arItem["VALUES"]["MIN"]["VALUE"]) : round($arItem["VALUES"]["MIN"]["HTML_VALUE"])?></span>
									
									<span class="max"><?=empty($arItem["VALUES"]["MAX"]["HTML_VALUE"]) ? round($arItem["VALUES"]["MAX"]["VALUE"]) : round($arItem["VALUES"]["MAX"]["HTML_VALUE"])?></span>
		...
		*/
		...
		//not prices
		foreach($resItems as $key => $arItem)
		{
			if(
				empty($arItem["VALUES"])
				|| isset($arItem["PRICE"])
			...
			$counter++; 
			?>
			<div class="paramsBox<?=($counter>$arParams["FILTER_SHOW"])? " paramsBox_hidden hide": "";?>">
				<div class="paramsBoxTitle">
                    ...
                    <?if($arItem["NAME"] == "Цена (модуль askaron.fastprice)") {?>
                        <span>Розничная цена</span>
                    <?} else {?>
                        <span><?=preg_replace("/\[.*\]/", "", $arItem["NAME"])?></span>
                    <?}?>

				</div> <ins class="propExpander <?if ($arItem["DISPLAY_EXPANDED"]== "Y" || $arItem["NAME"] == "Цена (модуль askaron.fastprice)"):?>expanded<?else:?><?endif?>"></ins>
				<div class="params <?if ($arItem["DISPLAY_EXPANDED"]== "Y" || $arItem["NAME"] == "Цена (модуль askaron.fastprice)"):?> <?else:?>collapsed<?endif?>">
					<?
			...
			<? if(@$counter==$arParams["FILTER_SHOW"]): ?>
			<div class="paramsBox paramsBox__readmore">
				<div class="paramsBoxTitle">
		...
		<ul id="smartFilterControls">
            ...
            <?if(empty($secFilter['SECTION_ID'])) {?>
            <li><a id="del_filter_cat" href="<?=$APPLICATION->GetCurPage()?>"><span class="close"></span><?=GetMessage("CT_BCSF_DEL_FILTER")?></a>
            <?} else {?>
            <li><a id="del_filter" href="#"><span class="close"></span><?=GetMessage("CT_BCSF_DEL_FILTER")?></a>
            <?}?>
		</ul>
