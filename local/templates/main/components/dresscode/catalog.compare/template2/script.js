$(window).on("ready", function(){
	...
	(function($){
		...
		if(scrollTable.width() < $("#scrollTable").width()){
			topScroll.hide();
			$("#compareBlock .left .propList").css("margin-top", "10px");
		}else{
...
*/
		...
		$("#compareTools a.show").on("click",function(e){
			$("#compareCheck input:checked").each(function(index){
				...
				propList.find('[data-id="' + $(this).prop("id") + '"]').show();
			});
			e.preventDefault();
		});
		
		$('#btn-on').click(function(){
			$('#compareCheck input:checkbox').prop('checked', true);
		});
		
		$('#btn-off').click(function(){
			$('#compareCheck input:checkbox').prop('checked', false);
		});		
