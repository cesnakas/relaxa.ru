        ...
        <table class="stats">
            ...
            <? if (!empty($arSome)): ?>
                ...
                <? foreach ($arSome as $arProp): ?>
                    <? $set++ ?>
                    <tr<? if ($set % 2): ?> class="gray"<? endif; ?>>
                        <td class="name"><span><?= $arProp["NAME"] ?></span><? if (!empty($arProp["HINT"])): ?><a
                        ...
                        <td class="right">
                            <? if ($arProp["FILTRABLE"] == "Y" && !is_array($arProp["VALUE"]) && !empty($arProp["VALUE_ENUM_ID"]) && !empty($arResult["LAST_SECTION"])): ?>
                                <a href="<?= $arResult["LAST_SECTION"]["SECTION_PAGE_URL"] ?>?arrFilter_<?= $arProp["ID"] ?>_<?= abs(crc32($arProp["VALUE_ENUM_ID"])) ?>=Y&amp;set_filter=Y"
